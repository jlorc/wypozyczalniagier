    ...
    <table class="rented-table">
      ...
      <tbody>
        <tr v-for="item in list" v-if="list">
          ...
          <td>
            <span class="rented-table__delete" @click="deleteRentedGame(item.id, item.game_id)">
...
<script>
    ...
    mounted() {
		...
		methods: {
			init() {
				return FetchService.fetchData('http://localhost:3000/api/rented').then(data => {
					this.list = data;
				});
      },
			deleteRentedGame(id, game_id) {
				return FetchService.deleteData(`http://localhost:3000/api/rent/${id}`, {id: id, game_id: game_id}).then(data => {
					return FetchService.fetchData('http://localhost:3000/api/rented').then(data => {
						this.list = data;
					});
				})
